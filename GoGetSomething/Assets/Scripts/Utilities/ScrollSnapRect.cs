...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    private void SetPagePositions()
    {
        ...
        if (_horizontal)
        {
            ...
            //            var ar = AspectRatio.GetAspectRatio(Screen.width, Screen.height, true);

            //            var result = ar.x/ar.y;
            ...
            //                Debug.Log("PHONE!");
            //                // it's a phone
            ////                width = (int)(_scrollRectRect.rect.width * 0.75f); //Mobil
            ////                offsetX = (int)(width * 0.5f); //Mobil
            //            }
            //            else
            //            {
            //                Debug.Log("TABLET!");
            //                // it's tablet
            ////                width = (int)(_scrollRectRect.rect.width * 0.9f); //Tablet
            ////                offsetX = (int)(width * 0.8f); //Tablet
            //            }
    ...
    private void SetPage(int aPageIndex)
    {
        ...
        OnChangePanel(_currentPage);
        UpdateCircles();
        UpdateDrawedCircles(aPageIndex);

        if(_endButton != null) _endButton.interactable = _currentPage >= _pageCount -1;
    }
    ...
    //Axel Changes
    private void UpdateDrawedCircles(int indexPage)
    {
        if (_drawedCircles.Length == 0) return;
        if (_previousPage == indexPage) return;
        _previousPage = indexPage;

        if (indexPage == 0)
        {
            StartCoroutine(EffectDrawedCircles(0));
        }
        else if (indexPage == 4)
        {
            StartCoroutine(EffectDrawedCircles(1));
        }
    }

    private IEnumerator EffectDrawedCircles(int i)
    {
        _drawedCircles[i].fillAmount = 0;

        yield return new WaitForSeconds(1);

        DOTween.To(() => _drawedCircles[i].fillAmount, value => _drawedCircles[i].fillAmount = value, 1, 1f).SetEase(Ease.InOutQuart);
    }

    private void UpdateCircles()
    {
        DOTween.Kill("Coloring");
        for (int i = 0; i < _circles.Length; i++)
        {
            if (i == _currentPage)
            {
                _circles[i].DOColor(_selectedColor, 0.5f).SetEase(Ease.InOutSine).SetId("Coloring");
            }
            else
            {
                _circles[i].DOColor(_unselectedColor, 0.5f).SetEase(Ease.InOutSine).SetId("Coloring");
            }
        }
    }

    public virtual void OnChangePanel(int newPanelId)
    {
